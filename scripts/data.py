...
def validate_svg(svg):
    if len(svg.encode()) > 1_000_000 or re.search(r'<!DOCTYPE|<!ENTITY', svg, re.I):
    ...
    for element in root.iter():
        ...
        if re.search(r'@import|javascript:|expression\s*\(|https?://|data:', content, re.I):
            raise ValueError('External or active SVG content')
        for ref in re.findall(r'url\s*\(([^)]*)\)', content, re.I):
...
def rows_from_baseline():
    ...
    for row in rows:
        if not re.fullmatch('[A-Z]{2}', row['cca2']) or any(not str(v).strip() or str(v).lower() in {'null', 'undefined', 'none'} for v in row.values()):
...
def build(refresh=False, check=False):
    ...
    if refresh:
        ...
        overrides = {code: {'svg': svg, 'source': f'https://flagcdn.com/{code.lower()}.svg',
    ...
    for row in rows:
        ...
        countries.append({'id': code, 'name': row['common'], 'official': row['official'],
                          'aliases': policy['aliases'].get(code, []), 'asset': asset,
    ...
    manifest = {'schema': 1, 'baseline_sha256': digest((ROOT / 'speedflags.db').read_bytes()),
                'policy': policy['notes'], 'countries': countries, 'shared_groups': duplicates}
    version = digest(json.dumps(manifest, sort_keys=True).encode())[:16]
    manifest['version'] = version
    with tempfile.TemporaryDirectory(dir=ROOT / 'data') as temp:
        stage = Path(temp)
        flags = stage / 'flags'; flags.mkdir()
        ...
        db.execute('CREATE TABLE countries (id TEXT PRIMARY KEY, name TEXT NOT NULL, official TEXT NOT NULL, aliases TEXT NOT NULL, asset TEXT NOT NULL, starter INTEGER NOT NULL)')
        db.executemany('INSERT INTO countries VALUES (?,?,?,?,?,?)', [(c['id'], c['name'], c['official'], json.dumps(c['aliases']), c['asset'], c['starter']) for c in countries])
        db.execute('CREATE TABLE metadata (version TEXT NOT NULL)')
        db.execute('INSERT INTO metadata VALUES (?)', (version,))
        db.commit(); db.close()
        output = ROOT / 'data/generated'; asset_output = ROOT / 'frontend/public/flags'
        if check:
            for filename in ('manifest.json', 'countries.db'):
                if filename.endswith('.db') and (output / filename).exists():
                    with sqlite3.connect(stage / filename) as new, sqlite3.connect(f'file:{output / filename}?mode=ro', uri=True) as old:
                        equal = list(new.iterdump()) == list(old.iterdump())
                else:
                    equal = (output / filename).exists() and (stage / filename).read_bytes() == (output / filename).read_bytes()
                if not equal:
                    raise ValueError(f'Stale generated {filename}; run python3 scripts/data.py')
            if not asset_output.exists() or {p.name: p.read_bytes() for p in flags.iterdir()} != {p.name: p.read_bytes() for p in asset_output.iterdir()}:
            ...
            output.mkdir(exist_ok=True); asset_output.mkdir(parents=True, exist_ok=True)
            for filename in ('manifest.json', 'countries.db'):
        ...
        cards = ''.join(f'<figure><img src="../frontend/public/flags/{c["asset"]}.svg" alt=""><figcaption>{html.escape(c["id"] + " · " + c["name"])}</figcaption></figure>' for c in countries)
        if not check:
            (ROOT / 'docs/flags.html').write_text('<!doctype html><html lang="en"><meta charset="utf-8"><title>Flag audit</title><style>body{font:14px system-ui;display:grid;grid-template-columns:repeat(5,1fr);gap:12px}figure{margin:0;padding:12px;border:1px solid #ccc}img{width:100%;height:100px;object-fit:contain}figcaption{margin-top:8px}</style>' + cards + '</html>')
