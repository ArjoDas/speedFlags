...
## Daily Challenge scoring and sharing

- Replaced the 30s/+2s Challenge with 30 distinct daily flags from the full collection, ordered by a stable hash of UTC date, dataset version and asset ID. The separate warm-up never appears in that day's scored deck. Starting a prepared challenge after its UTC date changes is rejected; an already-started round retains its original date.
- The server owns start/end timestamps, advances on every answer/skip, freezes elapsed time at completion, and adds five seconds for each incorrect/skipped answer. Displayed results round elapsed time up to whole seconds; the live clock interpolates from the server start timestamp. Early exits and session expiry remain incomplete and unshareable.
